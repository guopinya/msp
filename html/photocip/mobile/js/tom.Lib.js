...
//css3 event
    function prefixEvent(t) {
        if (t) {
            var e = document.createElement("div"), n = {Webkit: "webkit", Moz: "", O: "o"};
            for (var i in n) if (void 0 != e.style[i + "TransitionProperty"]) return n[i] + String.fromCharCode(t[0].charCodeAt() - 32) + t.slice(1)
        }
    }
    ...
    _.cropImage = function (opts) {
        ...
        var G = {preview: null, moveX: 0, moveY: 0, scale: 1, rotate: 0, ratio: opts.devicePixelRatio},
            transform = _.prefixStyle("transform"),
            canvas = opts.canvas, ctx = canvas.getContext('2d'), _x, _y, _scale, offset = $(canvas).offset(),
            $bindPreview = opts.bindPreview || $('<img/>'), canMove;
        $(canvas).prop({
            width: opts.cropWidth * opts.devicePixelRatio,
            height: opts.cropHeight * opts.devicePixelRatio
        }).css({
...
//组动画   循环替换一组元素的class 实现循环式替换动画效果，frameClass可以是一组不同的，也可以是同一个
    ...
    _.animateGroup = function (opts) {
        ...
        var animArr = opts.group, reset, duration = opts.duration + (opts.gap || 0), curEl = animArr.eq(0),
    ...
    _.fillNumByWord = function (num, word, len) {
        return (Array(len + 1).join(word)).slice(0, len - (num + '').length) + num
    };
    _.browser = {
        versions: function () {
